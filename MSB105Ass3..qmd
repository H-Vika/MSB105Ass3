...
Fra @fig-1 og @fig-2 ser vi at den generelle trenden viser en svak, men tydelig nedgang i andelen arbeidstakere som jobber i samme kommune som de bor i. Nedgangen er ikke dramatisk, men konsistent. Det betyr at folk i økende grad pendler ut av hjemkommunen sin, og at arbeidsmarkedene i regionen blir mer integrerte over tid. 
...
plot_pendlere <- function(data, knr, grense = 1) {
  ...
  data_plot <- data |>
    filter(
      bkom == knr,                # bostedskommune du vil se på
      !akom %in% c("k0000"),
      akom != knr, # fjern totaler
      andel >= grense             # grense filtrerer bort små tall
    ) |>
    mutate(
      Arbeidssted = akom_navn     # nytt navn til legend
    )
